...
static int mxc_bt_rfkill_probe(struct platform_device *pdev)
{
	...
	data->wake_bt_gpio = of_get_named_gpio(np, "wake-bt-gpios", 0);
	if (data->wake_bt_gpio == -EPROBE_DEFER) {
		printk(KERN_INFO "mxc_bt_rfkill: wake_bt_gpio gpio not ready, need defer\n");
		return -EPROBE_DEFER;
	}
	if (gpio_is_valid(data->wake_bt_gpio)) {
		printk(KERN_INFO "wake bt gpio is:%d\n", data->wake_bt_gpio);
		rc = devm_gpio_request_one(&pdev->dev,
								data->wake_bt_gpio,
								GPIOF_OUT_INIT_HIGH,
								"Wake BT Gpio");
		if (rc) {
			dev_err(&pdev->dev, "unable to get wake-bt-gpios\n");
			goto error_request_gpio;
		}
	 

	} else  {
		printk("wake bt gpio not valid (%d)!\n", data->wake_bt_gpio);
	}

	data->wake_host_gpio = of_get_named_gpio(np, "wake-host-gpios", 0);
	if (data->wake_host_gpio == -EPROBE_DEFER) {
		printk(KERN_INFO "mxc_bt_rfkill: bt wake host gpio not ready, need defer\n");
		return -EPROBE_DEFER;
	}
	if (gpio_is_valid(data->wake_host_gpio)) {
		printk(KERN_INFO "wake host gpio is:%d\n", data->wake_host_gpio);
		rc = devm_gpio_request_one(&pdev->dev,
								data->wake_host_gpio,
								GPIOF_IN,
								"BT wake host");
		if (rc) {
			dev_err(&pdev->dev, "unable to get wake-host-gpios\n");
			goto error_request_gpio;
		}
	 

	} else  {
		printk("bt wake host gpio not valid (%d)!\n", data->wake_bt_gpio);
	}
	printk("rfkill %s 006 \n", __func__);
	gpio_set_value_cansleep(data->bt_power_gpio, 0);
	msleep(100);
	...
	rfk = rfkill_alloc("mx8_bt", &pdev->dev, RFKILL_TYPE_BLUETOOTH,
